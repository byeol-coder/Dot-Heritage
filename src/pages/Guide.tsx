...
export function Guide({ heritageId, mode, onBack }: Props) {
  ...
  return (
    <div className={styles.page}>
      ...
      {mode === 'school' && (
        <div className={styles.schoolProgress}>
          {['전체 형태', '중앙 창', '하단 기단', '상단 구조', '퀴즈'].map((step, i) => (
            <div key={i} className={`${styles.step} ${currentSlide >= i ? styles.stepDone : ''}`}>
              <div className={styles.stepNum}>{currentSlide > i ? '✓' : i + 1}</div>
